package com.muli.picturemanage.service.impl;
...
import com.muli.picturemanage.manager.upload.FilePictureUpload;
import com.muli.picturemanage.manager.upload.PictureUploadTemplate;
import com.muli.picturemanage.manager.upload.UrlPictureUpload;
 ...
 * @createDate 2025-02-08 21:55:25
 */
@Service
@Slf4j
public class PictureServiceImpl extends ServiceImpl<PictureMapper, Picture>
     ...
     */
    @Override
    public Page<PictureVO> listPictureVOByPageLocalCache(PictureQueryRequest pictureQueryRequest, HttpServletRequest request) {
     ...
     */
    @Override
    public Page<PictureVO> listPictureVOByPageRedis(PictureQueryRequest pictureQueryRequest, HttpServletRequest request) {
        long current = pictureQueryRequest.getCurrent();
        long size = pictureQueryRequest.getPageSize();
        // 构建缓存 key
        String queryCondition = JSONUtil.toJsonStr(pictureQueryRequest);
     ...
     */
    @Override
    public Page<PictureVO> listPictureVOByPageLocalCacheAndRedis(PictureQueryRequest pictureQueryRequest, HttpServletRequest request) {
        long current = pictureQueryRequest.getCurrent();
        long size = pictureQueryRequest.getPageSize();
        // 构建缓存 key
        String queryCondition = JSONUtil.toJsonStr(pictureQueryRequest);
...
//        }
        ...
        try {
            ...
            // 清除流览图
            String viewUrl = oldPicture.getViewUrl();
            if (StrUtil.isNotBlank(viewUrl)) {
                cosManager.deleteObject(new URL(viewUrl).getPath());
            }
            // 清理缩略图
            ...
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "格式错误的url");
        }
        ...
        boolean update = spaceService.lambdaUpdate()
                ...
                .update();
        if(!update) {
            throw new BusinessException(ErrorCode.OPERATION_ERROR, "额度计算失败");
        }
    }
